...
import {
  ...
  DialogActions, TextField, Chip, IconButton, Alert, CircularProgress,
} from '@mui/material';
...
const InterestRateManagement: React.FC<Props> = ({ darkMode }) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingRate) {
        await axios.put(`${API_URL}/admin/interest-rates/${editingRate.id}`, { interest_rate: parseFloat(formData.interest_rate), is_active: true }, { headers });
      } else {
        await axios.post(`${API_URL}/admin/interest-rates`, { loan_purpose: formData.loan_purpose, interest_rate: parseFloat(formData.interest_rate) }, { headers });
      }
      handleCloseDialog();
      fetchRates();
    } catch (err: any) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${API_URL}/admin/interest-rates/${id}`, { headers: { Authorization: `Bearer ${token}` } });
      fetchRates();
    } catch (err: any) {
  ...
  return (
    <Box sx={{ flexGrow: 1, p: 2, overflow: 'auto' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
        <Button variant="text" onClick={() => navigate('/admin-dashboard')} startIcon={<ArrowBack />}>
          Back to Dashboard
        </Button>
      ...
      <Paper sx={{ bgcolor: paperBg, color: textColor, p: 3 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
          ...
          <Button variant="contained" startIcon={<AddIcon />} onClick={() => handleOpenDialog()}>Add Rate</Button>
        </Box>
              ...
              <TableBody>
                {rates.map((rate) => (
                  <TableRow key={rate.id} hover>
                    ...
                    <TableCell>
                      <Chip label={rate.is_active ? 'Active' : 'Inactive'} color={rate.is_active ? 'success' : 'default'} size="small" />
                    </TableCell>
                    <TableCell sx={{ color: textColor }}>{new Date(rate.created_at).toLocaleDateString()}</TableCell>
                    <TableCell>
                      <IconButton size="small" onClick={() => handleOpenDialog(rate)} sx={{ color: textColor }}>
                        <EditIcon fontSize="small" />
                      </IconButton>
                      <IconButton size="small" onClick={() => handleDelete(rate.id)} sx={{ color: 'error.main' }}>
      ...
      <Dialog open={dialogOpen} onClose={handleCloseDialog} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Box sx={{ pt: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
            <TextField fullWidth label="Loan Purpose" value={formData.loan_purpose} onChange={(e) => setFormData({ ...formData, loan_purpose: e.target.value })} disabled={!!editingRate} helperText={editingRate ? 'Cannot change loan purpose after creation' : ''} />
            <TextField fullWidth label="Interest Rate (%)" type="number" value={formData.interest_rate} onChange={(e) => setFormData({ ...formData, interest_rate: e.target.value })} inputProps={{ step: '0.01', min: '0', max: '100' }} />
          </Box>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleCloseDialog}>Cancel</Button>
          <Button onClick={handleSubmit} variant="contained">{editingRate ? 'Update' : 'Create'}</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
